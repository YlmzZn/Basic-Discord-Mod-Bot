...
import traceback
import slash_paginator


class SlashPublicMenu(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot

    @app_commands.command(name="help", description="Command menu")
    async def slash_public_menu(self, interaction: discord.Interaction):
        try:
            img = "image url"
            ...
            await slash_paginator.Simple(
                ...
                InitialPage=0,
                timeout=42069).start(interaction, pages=embeds)
        except:
...
async def setup(bot: commands.Bot):
    await bot.add_cog(SlashPublicMenu(bot))
